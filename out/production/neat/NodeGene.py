...
class NodeGene:
    ...
    def copy_node(self):
        copied = NodeGene(self._type, self._id)
        for con in self._connections_in:
            copied.append_con_in(con)
        for con in self._connections_out:
            copied.append_con_out(con)
        copied._disabled_connection = copy.deepcopy(self._disabled_connection)
        return copied
